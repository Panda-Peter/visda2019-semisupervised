...
def make_dataset(root, list_path, labels):
    ...
    for i in range(len(list_path)):
        images[labels[i]].append(os.path.join(root, list_path[i]))
    return images


class OnlineLoader(data.Dataset):
    def __init__(self, root, list_path, labels, transform=None, teacher_mode=False, loader=default_loader):
    ...
    def __getitem__(self, index):
        label, imgid = index
        path = self.imgs[label][imgid]

        img = self.loader(path)

        if self.teacher_mode == False:
            ...
            if self.transform is not None:
                img = self.transform(img)
                tea_img = self.transform(tea_img)
            return img, tea_img, label
