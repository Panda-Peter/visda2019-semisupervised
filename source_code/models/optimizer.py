    ...
    def setup_optimizer(self, netG, netE):
        ...
        if cfg.SOLVER.TYPE == 'sgd':
            ...
            self.optimizer = torch.optim.RMSprop(params,
                                                 lr=cfg.SOLVER.BASE_LR, alpha=0.9, eps=0.001, weight_decay=0.0,
                                                 momentum=cfg.SOLVER.MOMENTUM)
        else:
        ...
        if cfg.SOLVER.LR_POLICY == 'fixed':
            ...
            self.scheduler = WarmupMultiStepLR(
                ...
                warmup_iters=cfg.SOLVER.WARM_UP_ITERS,
                warmup_method=cfg.SOLVER.WARM_UP_METHOD,
            )
        elif cfg.SOLVER.LR_POLICY == 'cosine':
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=
            cfg.SOLVER.MAX_EPOCH, eta_min=0.00005)
        else:
            raise NotImplementedError
